...
export function AppointmentPageHeader({
    ...
    command && statusTone === "success"
      ...
      : command && statusTone === "warning"
        ...
        : command
          ...
          : statusTone === "success"
      ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-700 dark:border-white/15 dark:bg-white/10 dark:text-neutral-200"
      : statusTone === "warning"
        ? "border-amber-500/20 bg-amber-500/10 text-amber-700 dark:border-white/15 dark:bg-white/10 dark:text-neutral-200"
  ...
  return (
    <section
      className={
        command
          ? "relative overflow-hidden rounded-[1.65rem] border border-white/10 bg-[linear-gradient(125deg,#0c0c0c_0%,#161616_52%,#222222_100%)] text-white shadow-[var(--shadow-lg)]"
      ...
      <div className={`relative flex flex-wrap items-center justify-between gap-5 ${command ? "px-5 pb-5 pt-6 lg:px-7 lg:pt-7" : "px-4 py-4 lg:px-5"}`}>
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <p className={`text-[10px] font-semibold uppercase tracking-[0.18em] ${command ? "text-neutral-400" : "text-[var(--color-primary-h)]"}`}>
              {eyebrow}
            </p>
            {status ? (
          ...
          <div className={`mt-2 max-w-3xl text-sm leading-6 ${command ? "text-neutral-400" : "text-[var(--color-text-muted)]"}`}>
            {description}
          </div>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {actions.map((action) => (
              <Link
                ...
                className={
                  action.primary
                    ? `rounded-xl px-3.5 py-2.5 text-xs font-semibold transition ${command ? "bg-white text-black hover:bg-neutral-200" : "bg-[var(--color-primary)] text-[var(--color-primary-fg)] hover:bg-[var(--color-primary-h)]"}`
